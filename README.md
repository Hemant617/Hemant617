# 👋 Hi, I'm Hemant Kaushal

<div align="center">
  
[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-hemant617.github.io-F85D7F?style=for-the-badge)](https://hemant617.github.io/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/hemantkaushal)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:hemuh877@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Hemant617)

[![Recruiters Click Here](https://img.shields.io/badge/👔_RECRUITERS_CLICK_HERE-Quick_Overview-00C853?style=for-the-badge)](./RECRUITER.md)

</div>

## 🛡️ Cybersecurity Analyst | 📊 Data Analyst | 🎓 B.Tech CSE Student

> *"Turning threats into insights, vulnerabilities into solutions"*

### 🌟 [View My Portfolio Website →](https://hemant617.github.io/)
### 👔 [Recruiter Quick Overview →](./RECRUITER.md)

---

### 🚀 About Me

I'm a motivated **Cybersecurity Analyst** and **Data Analyst** with hands-on experience in incident response, threat detection, and forensic analysis. Currently pursuing B.Tech in Computer Science & IT at Graphic Era Hill University, I specialize in security automation, log forensics, and data-driven threat intelligence.

- 🔐 **Security Focus**: SOC operations, incident response, vulnerability assessment, penetration testing
- 📊 **Analytics Expertise**: Log analysis, pattern detection, anomaly identification, forensic technology
- 🐍 **Automation**: Python scripting for security automation and threat detection
- 🌍 **Location**: Dehradun, Uttarakhand, India
- 💼 **Open to**: Remote SOC Analyst, Incident Response, Junior Security Analyst, Data Analyst roles

📋 **[My Career Action Plan](./CAREER_ACTION_PLAN.md)** - Roadmap to landing my dream cybersecurity role

---

### 🛠️ Technical Arsenal

#### Security Tools & Platforms
![Kali Linux](https://img.shields.io/badge/Kali_Linux-557C94?style=flat-square&logo=kali-linux&logoColor=white)
![Wireshark](https://img.shields.io/badge/Wireshark-1679A7?style=flat-square&logo=wireshark&logoColor=white)
![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6633?style=flat-square&logo=burp-suite&logoColor=white)
![Metasploit](https://img.shields.io/badge/Metasploit-2596CD?style=flat-square&logo=metasploit&logoColor=white)
![Nmap](https://img.shields.io/badge/Nmap-0E83CD?style=flat-square&logo=nmap&logoColor=white)

#### Programming & Scripting
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=c%2B%2B&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)

#### Data Analysis & Tools
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoft-excel&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)

#### Core Competencies
- **Incident Response & Forensics**: Log forensics, IoC identification, threat hunting, evidence preservation
- **Network Security**: TCP/IP analysis, packet capture, firewall configuration, VPN implementation
- **Vulnerability Assessment**: Web app security testing (XSS, IDOR, SQL injection), penetration testing
- **Data Analytics**: Pattern recognition, anomaly detection, statistical analysis, forensic technology
- **Automation**: Python scripting for detection automation, SIEM workflows, API security testing

---

### 🎯 Featured Security Projects

#### 🛡️ [AI Zero-Trust Security Orchestrator](https://github.com/Hemant617/ai-zerotrust-security-orchestrator)
**Enterprise-grade AI-powered Zero-Trust Security Platform**
- 🤖 Automated threat detection with behavioral analysis
- 🔐 Quantum-resistant cryptography implementation
- 🚨 Real-time incident response automation
- 📊 SOC automation and orchestration
- **Tech Stack**: Python, AI/ML, Zero-Trust Architecture

#### ☁️ [VirusTotal SaaS Platform](https://github.com/Hemant617/virustotal-saas-platform)
**Production-ready SaaS platform for VirusTotal scanning**
- 👥 User authentication and subscription management
- 🤝 Team collaboration features
- 📈 Comprehensive scan analytics
- 🔌 RESTful API integration
- **Tech Stack**: Python, FastAPI, PostgreSQL, Docker

#### 🛡️ [Threat Log Analyzer](https://github.com/Hemant617/threat-log-analyzer)
**Python-based security tool for analyzing system logs and detecting threats**
- 🔍 Detects 8+ threat types: Brute force, SQL injection, XSS, malware signatures
- 📊 Real-time monitoring with instant alerts
- 📈 Generates comprehensive HTML/JSON reports with severity scoring
- 🎯 IP tracking and security recommendations
- **Tech Stack**: Python, Watchdog, Regex, JSON

#### 🦠 [VirusTotal Automated Scanner](https://github.com/Hemant617/virustotal-automated-scanner)
**Automated security scanner using VirusTotal API for comprehensive threat analysis**
- 🔐 Scans files, URLs, domains, and IP addresses
- 🤖 70+ antivirus engine detection
- 📊 Batch processing with automated rate limiting
- 📄 Beautiful HTML reports with detection statistics
- **Tech Stack**: Python, VirusTotal API v3, Requests, Docker

#### 🤖 [SOC Automation Toolkit](https://github.com/Hemant617/soc-automation-toolkit)
**Enterprise SOC automation scripts and tools**
- 🔍 Automated threat detection and hunting
- 🚨 Incident response automation
- 📊 SIEM integrations (Splunk, Elastic, QRadar)
- 🎯 Alert enrichment and triage
- **Tech Stack**: Python, SIEM APIs, Threat Intelligence

---

### 🏆 Security Research & CTF

#### 🔬 [Security Research](https://github.com/Hemant617/security-research)
**Independent security research and vulnerability analysis**
- 🐛 Vulnerability discoveries and responsible disclosure
- 📝 Security advisories and proof-of-concepts
- 🔐 Web, mobile, cloud, and blockchain security research
- 🛠️ Custom security tools and automation

#### 🏆 [CTF Writeups](https://github.com/Hemant617/ctf-writeups)
**Capture The Flag solutions and methodologies**
- 🌐 Web exploitation (SQL injection, XSS, SSRF)
- 💻 Binary exploitation and reverse engineering
- 🔐 Cryptography challenges
- 🔍 Digital forensics and steganography
- **Platforms**: HackTheBox, TryHackMe, CTFtime

---

### 🔬 Hands-On Labs & Projects

#### Telemetry Incident Response & Detection Lab
**Simulated industrial IoT security incident response**
- 🏭 Dockerized Flask application mimicking SCADA/IoT systems
- 🤖 Python-based automated threat detection scripts
- 🎯 Identifies bot activity and lateral movement patterns
- 📋 Formal SOC-style incident response reports
- **Tech Stack**: Docker, Python, Flask, JSON, Regex

#### Home Network Security Hardening Lab
**Practical network defense and penetration testing**
- 🔍 Network security audit with Nmap and Wireshark
- 🛡️ WPA3 implementation, MAC filtering, network isolation
- 🎯 Web app penetration testing (OWASP Juice Shop, DVWA)
- 📝 Documented exploits with remediation strategies
- **Tech Stack**: Kali Linux, Nmap, Wireshark, Burp Suite, OWASP ZAP

#### Python Security Automation Framework
**Modular security automation and monitoring system**
- 🤖 Automated log analysis and anomaly detection
- 🚨 Real-time monitoring for brute-force attacks
- 📊 Automated reporting with severity classification
- 🔗 JSON parsing for SIEM log ingestion
- **Tech Stack**: Python, JSON, Regex, Automation

---

### 📜 Certifications

<div align="center">

| Certification | Issuer | Date | Skills |
|--------------|--------|------|--------|
| 🎓 **Deloitte Cyber Job Simulation** | Forage | Nov 2025 | Incident Response, Log Forensics, Threat Analysis |
| 🎓 **Deloitte Data Analytics Job Simulation** | Forage | Nov 2025 | Data Analysis, Forensic Technology, Pattern Detection |
| 🎓 **Introduction to Cybersecurity** | Cisco Networking Academy | Nov 2025 | Network Security, Cyber Threats, Security Fundamentals |

</div>

📂 **[View All Certifications →](https://github.com/Hemant617/certifications)**

---

### 💼 Professional Experience

#### 🔐 Cybersecurity Analyst Virtual Internship | Deloitte (Forage)
*November 2025*

- 🔍 Investigated simulated security incidents analyzing telemetry logs and web request data
- 🎯 Identified high-risk IoCs including suspicious authentication patterns and unauthorized access
- 📋 Produced comprehensive SOC-style incident reports with risk assessments
- 🚨 Developed technical handover documentation with complete remediation workflows
- ⚡ Demonstrated rapid threat detection under simulated production constraints

#### 📊 Data Analytics Virtual Internship | Deloitte (Forage)
*November 2025*

- 📈 Analyzed complex telemetry and log datasets to identify patterns and anomalies
- 🔬 Applied forensic data analysis techniques to investigate credential misuse
- 🎯 Prioritized findings based on data-driven risk assessment
- 📝 Created comprehensive analytical reports with actionable recommendations
- 💡 Extracted meaningful insights from large, unstructured datasets

---

### 📊 GitHub Stats

<div align="center">

![Hemant's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Hemant617&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&title_color=F85D7F&icon_color=F8D866)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Hemant617&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&title_color=F85D7F)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Hemant617&theme=radical&hide_border=true&background=0D1117&ring=F85D7F&fire=F8D866&currStreakLabel=F8D866)

</div>

---

### 🎓 Education

**Bachelor of Technology (B.Tech) - Computer Science & IT**  
*Graphic Era Hill University, India* | 2024 - Present

**Relevant Coursework**: Computer Networks, Operating Systems, Cybersecurity Fundamentals, Software Engineering, Database Security, Cryptography, Data Structures & Algorithms

---

### 🌟 What I'm Currently Working On

- 🔐 Building advanced SIEM detection rules and automation scripts
- 🐍 Developing Python-based security tools for threat intelligence
- 📚 Studying for CompTIA Security+ and CEH certifications
- 🔬 Creating more hands-on cybersecurity labs and projects
- 📊 Enhancing data analytics skills for security operations
- 🏆 Participating in CTF competitions and bug bounty programs

---

### 💡 Skills Highlights

```python
security_analyst = {
    "incident_response": ["Log Forensics", "IoC Identification", "Threat Hunting"],
    "network_security": ["Packet Analysis", "Firewall Config", "VPN Implementation"],
    "vulnerability_assessment": ["Web App Testing", "Penetration Testing", "Security Audits"],
    "data_analytics": ["Pattern Recognition", "Anomaly Detection", "Statistical Analysis"],
    "automation": ["Python Scripting", "SIEM Workflows", "Detection Automation"],
    "soft_skills": ["Technical Reporting", "Problem Solving", "Team Collaboration"]
}
```

---

### 📫 Let's Connect!

I'm actively seeking **remote opportunities** in:
- 🛡️ SOC Analyst / Security Operations
- 🚨 Incident Response Analyst
- 🔐 Junior Security Analyst
- 📊 Data Analyst (Security/Forensics focus)
- 🔬 Threat Intelligence Analyst

**Email:** hemuh877@gmail.com  
**Portfolio:** [hemant617.github.io](https://hemant617.github.io/)  
**LinkedIn:** [linkedin.com/in/hemantkaushal](https://linkedin.com/in/hemantkaushal)

---

*💼 Recruiters: Check out my [Quick Overview for Hiring Managers](./RECRUITER.md) for a streamlined view of my qualifications and top projects.*
